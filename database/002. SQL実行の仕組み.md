...
1. ネストループ結合（テーブルサイズが小さい場合）
2. ハッシュ結合（結合キーにインデックス無、ハッシュマップが収まる十分なメモリ容量あり）
...
#### ハッシュ結合（Hash JOIN）
- ハッシュマップで計算量`O(1)`でJOIN先のレコードが見つかるような仕組み。
- **インデックスの代わりにハッシュマップをメモリ上に作成＆利用**という感じ。
- このハッシュテーブルは通常、`Map（キーと値のペア）`形式で格納されます。
