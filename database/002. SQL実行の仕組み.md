...
<img src="https://github.com/daisuketakakuwa/learning-stack/assets/66095465/7083dd68-f425-4909-a741-7e83d02726f2" width="50%" />
...
　↓<br/>
JOINのアルゴリズムは主に３つ（MySQLはNestのみ採用）。<br/>**※どれを使うは都度クエリオプティマイザが決める。
...
```
👉ネストループ結合が使われやすい可能性（＝メモリに収まる/ディスクIOが発生しない傾向がある）
```
✅外側テーブルのデータが比較的小さい
✅内側テーブルの結合キーにインデックスが存在している
✅メモリ設定（例: work_mem）が十分で、結果がメモリ内に収まりそうな場合。
...
- 👉**ソートが重い処理になりがちなので、ソート用のインデックス有無が重要**
